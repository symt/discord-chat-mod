  ...
  public void onMessageReceived(MessageReceivedEvent event) {
    ...
    if (!DiscordChatMod.jda.getSelfUser().equals(author)) {
      if (channel.getType() == ChannelType.TEXT) {
        ...
        if (messageContents[0].equals(DiscordChatMod.jda.getSelfUser().getAsTag()) && channel
            .getName().equalsIgnoreCase("bot-communications")) {
          if (queue.isEmpty() || !queue.getLast().equals(msg)) {
            DiscordChatMod.lastUser = author;
            queue.add(msg);
            Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText(
                EnumChatFormatting.LIGHT_PURPLE + "From " + EnumChatFormatting.DARK_PURPLE + author
                    .getName() + EnumChatFormatting.LIGHT_PURPLE + ": " + EnumChatFormatting.RESET
                    + messageContents[1]));
          }
        }
