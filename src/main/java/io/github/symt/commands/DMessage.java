...
public class DMessage extends CommandBase {
  ...
  static void sendMessageToBot(String content, String prefix, String user, EntityPlayer player) {
    if (content.contains("~=~")) {
      player.addChatMessage(new ChatComponentText(EnumChatFormatting.GREEN
          + "Hold up there, you aren't allowed to use \"~=~\" because it is important!!"));
    } else {
      ...
      player.addChatMessage(new ChatComponentText(
          EnumChatFormatting.LIGHT_PURPLE + "To " + EnumChatFormatting.DARK_PURPLE + user
  ...
  static void sendMessageToUser(String content, User user, EntityPlayer player) {
    ...
    player.addChatMessage(new ChatComponentText(
        EnumChatFormatting.LIGHT_PURPLE + "To " + EnumChatFormatting.DARK_PURPLE + user.getName()
            + EnumChatFormatting.LIGHT_PURPLE + ": " + EnumChatFormatting.RESET + content));
  }
  ...
  public void processCommand(final ICommandSender ics, String[] args) {
    if (ics instanceof EntityPlayer) {
      ...
      if (args.length >= 2) {
        ...
        if (!ID.contains("#")) {
          jda.getGuilds()
              .forEach(guild -> membersWithName.addAll(guild.getMembersByName(ID, true)));
          membersWithName.forEach(member -> usersWithName.add(member.getUser()));
        } else {
          jda.getUsers().stream().filter(user -> user.getAsTag().equalsIgnoreCase(ID)).findFirst()
        ...
        if (usersWithName.size() > 1) {
          ...
          if (usersWithName.isEmpty()) {
            player.addChatMessage(
                new ChatComponentText(EnumChatFormatting.GREEN + "No user found with that name"));
          } else {
            ...
            if (reqUser.isBot()) {
              ...
              link.getChatStyle().setChatClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL,
                  "https://github.com/symt/discord-chat-mod/blob/master/README.md"));
              if (mainGuild != null) {
                if (reqUser.getJDA().getGuilds().contains(mainGuild)) {
                  sendMessageToBot(content, reqUser.getAsTag(), reqUser.getName(), player);
                } else {
                  player.addChatMessage(new ChatComponentText(EnumChatFormatting.GREEN
                      + "The bot that you are requesting is not in the discord bot connector server. Open an issue at the ")
                      .appendSibling(link).appendSibling(new ChatComponentText(
                          EnumChatFormatting.GREEN + " and request for access")));
                }
              } else {
                player.addChatMessage(new ChatComponentText(EnumChatFormatting.GREEN
                    + "You are not in the discort bot connector server. Open an issue at the ")
                    .appendSibling(link).appendSibling(new ChatComponentText(
                        EnumChatFormatting.GREEN + " and request for access")));
              }
            } else {
        ...
        player.addChatMessage(new ChatComponentText(
            EnumChatFormatting.GREEN + "Please provide a valid user and message"));
      }
    }
  }
